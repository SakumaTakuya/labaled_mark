...
export class LabeledRenderer extends marked.Renderer
  implements HeadingRenderer, CodeRenderer, ImageRenderer, TableRenderer {
...
export class BorderTableRenderer implements TablecellRenderer {
  tablecell(
    content: string,
    flags: { header: boolean; align: 'center' | 'left' | 'right' | null }
  ): string {
...
export class CustomizableRenderer extends marked.Renderer {
  ...
  code(code: string, infostring: string, escaped: boolean): string {
    return (
      this.functions.renderCode?.code(code, infostring, escaped) ||
      super.code(code, infostring, escaped)
    );
  }

  blockquote(quote: string): string {
    return this.functions.renderBlockquote?.blockquote(quote) || super.blockquote(quote);
  }

  html(html: string): string {
    return this.functions.renderHtml?.html(html) || super.html(html);
  }
  ...
  heading(
    ...
    return (
      this.functions.renderHeading?.heading(text, level, raw, slugger) ||
      super.heading(text, level, raw, slugger)
    );
  }

  hr(): string {
    return this.functions.renderHr?.hr() || super.hr();
  }

  list(body: string, ordered: boolean, start: number): string {
    return (
      this.functions.renderList?.list(body, ordered, start) || super.list(body, ordered, start)
    );
  }

  listitem(text: string): string {
    return this.functions.renderListitem?.listitem(text) || super.listitem(text);
  }

  checkbox(checked: boolean): string {
    return this.functions.renderCheckbox?.checkbox(checked) || super.checkbox(checked);
  }

  paragraph(text: string): string {
    return this.functions.renderParagraph?.paragraph(text) || super.paragraph(text);
  }

  table(header: string, body: string): string {
    return this.functions.renderTable?.table(header, body) || super.table(header, body);
  }

  tablerow(content: string): string {
    return this.functions.renderTablerow?.tablerow(content) || super.tablerow(content);
  }
  ...
  tablecell(
    ...
    return (
      this.functions.renderTablecell?.tablecell(content, flags) || super.tablecell(content, flags)
    );
  }

  image(href: string | null, title: string | null, text: string): string {
    return this.functions.renderImage?.image(href, title, text) || super.image(href, title, text);
  }
}
